...
var sqlcore = require("./index");

var SQLError = sqlcore.SQLError;
var SQLDriver = sqlcore.SQLDriver;
var SQLClient = sqlcore.SQLClient;
...
// Normalize an array of type parsers `input` into an array of objects where
...
function PGSQLDriver(app, config) {
  SQLDriver.call(this, app, config);

  // This is gonna throw, as intended, if there is no "pg" package. The reason
  // is that we cannot recover from this. The user can pass `config.backend`,
  // if `pg` or 'pg-native' was already required from before.
  var internal = this._internal;
  if (internal.impl === null)
    internal.impl = require("pg");

  // PG type parsers.
  if (config.pgTypeParsers)
    internal.pgTypeParsers = normalizeTypeParsers(pgTypeParsers);
}
exports.PGSQLDriver = exclass({
  $extend: SQLDriver,
  $construct: PGSQLDriver,
...
function PGSQLClient(driver, impl) {
  var self = this;

  SQLClient.call(this, driver, impl);
  self._onQuery = PGSQLClient.prototype._onQuery.bind(this);
}
exports.PGSQLClient = exclass({
  $extend: SQLClient,
  $construct: PGSQLClient,
